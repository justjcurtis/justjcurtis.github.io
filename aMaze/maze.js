class Maze{
    constructor(w, h, cellSize, xoffset=0, yoffset=0){
        this.w = w;
        this.h = h;
    ...
    newBlankGrid(){
        ...
        for(let j = 0; j < this.rows; j++){
            for(let i = 0; i<this.cols; i++){
                result.push(new Cell(i, j, this.cellSize,result.length, this.xoffset, this.yoffset))
            }
        }
        return result;
    }
    ...
    checkNeighbors(cell, revisitRate){
        ...
        for(let i = neighbors.length; i>=0; i--){
            if(neighbors[i] != undefined && (random(0, 1)<map(revisitRate, 0,1,0,0.13) || !neighbors[i].visited)){
                continue;
            }
