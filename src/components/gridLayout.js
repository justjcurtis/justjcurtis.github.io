...
const GridLayout = ({ projects = [] }) => {
    ...
    useEffect(() => {
        if (currentProjects.current !== projects) {
    ...
    return (
        <div style={styles.container} className="bg-neutral-focus mt-[49px]">
            <div className="form-control bg-neutral-focus p-4 fixed z-[90] w-screen -ml-4 bg-opacity-60 backdrop-blur-md">
                <input ref={filterInputRef} onKeyUp={debounce(setFilter)} type="text" placeholder="Search projects" className="input input-bordered w-full" />
