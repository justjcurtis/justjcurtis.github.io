...
const HomePage = () => {
    ...
    return (
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} exit={{ opacity: 0 }} style={{ top: 0, height: '100svh', overflow: 'hidden', backgroundImage: `url(${images.HomeBackground})` }} className="select-none hero flex-1 fixed">
            <div style={dynamicStyles.hide} className="hero-overlay bg-opacity-60 backdrop-blur-sm"></div>
            {todaysGame && <todaysGame.game />}
            <div style={dynamicStyles.show} className="hero-overlay bg-opacity-60 backdrop-blur-sm"></div>
            <div style={dynamicStyles.show} className="hero-content text-center text-neutral-content mt-[100px]">
                <div className="max-w-md">
                    ...
                    <div style={styles.topContainer}>
                        <m.p initial={{ opacity: 0, x: -10, y: 2 }} animate={{ opacity: 1, x: 0, y: 0 }} transition={{ delay: 0.25, duration: (0.1 + (0.3 * Math.random())), ease: 'easeInOut' }} exit={{ opacity: 0, x: -10, y: 2 }} style={styles.subtext} className="text-sm text-left mb-5">Welcome to my personal site. Here you can find <Link to={'/projects'}><span className='text-secondary underline bg-neutral rounded px-1'>things I've made</span></Link>. There are links to my other places on the internet in the top right. You can see some info <Link to={'/about'}><span className='text-secondary bg-neutral underline rounded px-1'>about me</span></Link> too. <br /><br />Tripple tap anywhere to toggle this page & focus on {todaysGame.name}.</m.p>
